  ...
  return <div className="app">
    ...
    <main className="main">
      ...
      <div className="content workspace-content">
        ...
        {page === "team" && <><div className="page-heading"><div><span className="eyebrow">SHARED WORKSPACE</span><h1>Team</h1><p>Everyone here can manage the same orders and catalogue.</p></div><Button label="Add user" variant="primary" icon={<Plus size={17}/>} onClick={() => setDialog({ type: "user" })}/></div><section className="panel team-list">{data.users.map(user => <div className="team-row" key={user.id}><span className="avatar">{user.name.slice(0,1).toUpperCase()}</span><span><strong>{user.name}</strong><small>@{user.username}</small></span><span className="team-role">{user.is_admin ? "Owner" : "Member"}</span></div>)}</section></>}
      </div>
    </main>
